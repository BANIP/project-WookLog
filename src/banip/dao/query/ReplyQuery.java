...
public class ReplyQuery extends SQLQuery{
	...
	public ReplyQuery(ReplyBean bean){
		if(isNull(bean)) this.bean = new ReplyBean();
		else this.bean = bean;
	}
 ...
 */
	 ...
	 * @return 쿼리
	 */
	public String getModifyReplyQuery(User user){
		int replyID = bean.getREPLY_ID();
		String replyContent = bean.getREPLY_CONTENT();
		
		String query = String.format("call board_do_replymodify(%d,'%s','%s','%s')",replyID,replyContent, user.getName(), user.getPWD());
		return query;
	}
