...
public class BoardQuery extends SQLQuery{
	...
	public BoardQuery(BoardBean bean){
		if(isNull(bean)) this.bean = new BoardBean();
		else this.bean = bean;
	}
	...
	public String getSelectDetailQuery(int boardID){
		...
		String query = String.format("SELECT %s ", columnsName)
	 ...
	 * @return 쿼리
	 */
	public String getModifyBoardQuery(User user){
		int boardID = bean.getBOARD_ID();
		int categoryID = bean.getBOARD_CATEGORY_ID();
		String boardTitle = bean.getBOARD_TITLE();
		String boardContent = bean.getBOARD_CONTENT();

		String query = String.format("CALL BOARD_DO_MODIFY(%d,%d,'%s','%s','%s','%s')", boardID, categoryID, boardTitle, boardContent, user.getName(), user.getPWD() );
		return query;
	}
